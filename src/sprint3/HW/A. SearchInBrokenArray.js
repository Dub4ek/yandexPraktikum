...
/**
 * ПРИНЦИП РАБОТЫ
 * По условию задачи массив отсортирован, единственная трудность, что минимальный элемент сдвинут на какую-то величину. Идея решения задачи найти точку "перелома", т.е необходимо найти минимальный элемент в массиве. Так как массив отсортирован, то минимальный мы можем найти бинарным поиском. Потом уже зная "сдвиг" в массиве, используя опять же бинарный поиск найдем элемент в массиве.
 *
 * ДОКАЗАТЕЛЬСТВО КОРРЕКТНОСТИ
 * В условии задачи сказано, что алгоритм должен работать за O(logN), в разделе ВРЕМЕННАЯ СЛОЖНОСТЬ подробно описано и доказано, что требования по сложности выполняется
 *
 * ВРЕМЕННАЯ СЛОЖНОСТЬ
 * Бинарный поиск выполняется за O(logN). В данной задаче мы выполняем два бинарных поиска, поэтому общая сложность алгоритма O(2 * logN). Константы на больших значениях мы можем опускать, поэтому сложность финальная алгоритма O(logN).
 */
/**
